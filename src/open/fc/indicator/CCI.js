...
sap.ui.define(
  ...
  function(Control) {
    ...
    return Control.extend("open.fc.indicator.CCI", {
      metadata: {
        properties: {
          padding: "int",
          width: "int",
        ...
        aggregations: {
          points: { type: "open.fc.Point", multiple: true }
        }
      },
      ...
      onAfterRendering: function() {
        ...
        var iTimeframe = moment(
          aPoints.length > 1 ? aPoints[1].getX() : null
        ).diff(dMinX, "m");
        var dMaxX = moment(
          aPoints.length ? aPoints[aPoints.length - 1].getX() : null
        )
          .add(iTimeframe, "m")
          .toDate();
